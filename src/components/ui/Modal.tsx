...
export default function Modal({ title, children, onClose, size = 'md' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-navy-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
      <div className={`bg-white rounded-2xl shadow-xl w-full ${maxW} max-h-[88vh] flex flex-col overflow-hidden`}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-navy-100 flex-shrink-0">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded-lg bg-navy-50 hover:bg-navy-100 flex items-center justify-center text-navy-400 transition-colors">
            <X size={14} />
          </button>
        </div>
