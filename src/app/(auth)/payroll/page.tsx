...
import { fmtCurrencyShort } from '@/lib/utils'
...
export default async function PayrollPage() {
  ...
  const totalNet = (salary ?? []).reduce((s, r) => s + (r.net_salary ?? 0), 0)
  ...
  ;(salary ?? []).forEach(r => { const d = r.employee?.division ?? 'Unknown'; byDiv[d] = (byDiv[d] || 0) + (r.net_salary ?? 0) })
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        <div className="grid grid-cols-4 gap-3">
          ...
          <KPICard label="Avg salary/karyawan"  value={fmtCurrencyShort(avgSalary)} change="per bulan"  changeType="flat" accent="bg-blue-400" />
          ...
          <div className="card">
            ...
            <div className="card-body space-y-2">
              {divData.map(([div, total]) => (
                <InlineBar key={div} label={div} value={fmtCurrencyShort(total)} pct={Math.round(total / maxDiv * 100)} color="bg-navy-800" />
              ))}
            </div>
          </div>
          <div className="card">
            ...
            <div className="card-body">
              {[
                { label: 'Total gaji pokok', val: (salary ?? []).reduce((s, r) => s + r.basic_salary, 0) },
                { label: 'Total tunjangan',  val: (salary ?? []).reduce((s, r) => s + r.allowance, 0) },
                { label: 'Total lembur',     val: (salary ?? []).reduce((s, r) => s + r.overtime, 0) },
                { label: 'Total bonus',      val: (salary ?? []).reduce((s, r) => s + r.bonus, 0) },
                { label: 'Total potongan BPJS', val: (salary ?? []).reduce((s, r) => s + r.bpjs_ketenagakerjaan + r.bpjs_kesehatan, 0) },
              ].map(r => (
                <div key={r.label} className="flex justify-between items-center py-2 border-b border-navy-50 last:border-0">
                  <span className="text-[11.5px] text-navy-400">{r.label}</span>
        ...
        <div className="card overflow-x-auto">
          <div className="card-head">
            <span className="card-title">Detail salary — Mei 2026</span>
            ...
            <tbody>
              {(salary ?? []).map(s => (
                <tr key={s.id}>
                  ...
                  <td className="text-[11px] text-navy-400">{s.employee?.division}</td>
        ...
        <div className="grid grid-cols-2 gap-3">
          <InsightCard title="Payroll +4.2% MoM — in line dengan HC growth" text="Cost per head turun dari Rp 8.1 Jt ke Rp 7.6 Jt — tanda hiring lebih banyak di level junior. Positif untuk efisiensi cost." />
          <InsightCard title="Operations: cost per head tertinggi" text="4 karyawan senior di Operations punya cost per head tertinggi. Evaluasi apakah ROI-nya sebanding dengan output aktual." color="bg-blue-900/70" titleColor="text-blue-300" />
        </div>
      </div>
    </div>
  )
