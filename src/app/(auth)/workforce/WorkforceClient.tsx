...
export default function WorkforceClient({ employees: init }: Props) {
  ...
  const filtered = employees.filter(e =>
    ...
    (!fEntity || e.entity === fEntity) &&
    (!fDiv || e.division === fDiv)
  )

  // Division counts for chart
  const divCounts = Object.entries(
    active.reduce((acc: any, e) => {
  ...
  function handleExport() {
    const rows = filtered.map(e => ({
      ...
      'Gender': e.gender ?? '', 'Tgl Lahir': e.birth_date ?? '',
      'Join Date': e.join_date, 'End Date': e.end_date ?? '', 'Masa Kerja': calcYoS(e.join_date),
    }))
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setImporting(true); setImportMsg('Membaca file...')
    try {
      const buf = await file.arrayBuffer()
      const wb = XLSX.read(buf)
      const rows: any[] = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]])
      setImportMsg(`${rows.length} baris ditemukan. Mengupload...`)
      ...
      setImportMsg(`✓ ${data.count} karyawan berhasil diimport!`)
      // refresh
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-5 gap-3">
        ...
        <KPICard label="Aktif"            value={active.length}                       accent="bg-teal-400" change="+3 MoM" changeType="up" />
        ...
        <div className="card">
          ...
          <div className="card-body space-y-2">
            {divCounts.slice(0, 8).map(([div, count]) => (
              <InlineBar key={div} label={div as string} value={`${count}`}
                pct={Math.round((count as number / maxDiv) * 100)} color="bg-navy-800" />
            ))}
          </div>
        </div>
        ...
        <div className="col-span-3 space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center gap-1.5 bg-white border border-navy-100 rounded-lg px-3 py-1.5 flex-1 min-w-[180px]">
              <Search size={12} className="text-navy-400" />
              <input value={search} onChange={e=>setSearch(e.target.value)}
                placeholder="Cari nama, ID, divisi..." className="bg-transparent text-[11.5px] outline-none w-full text-navy-800 placeholder:text-navy-400" />
            </div>
            ...
            <div className="ml-auto flex items-center gap-2">
              {importMsg && (
                <span className={cn('text-[10.5px] font-medium', importMsg.startsWith('✓') ? 'text-teal-600' : importMsg.startsWith('✗') ? 'text-red-600' : 'text-amber-600')}>
                  {importMsg}
                </span>
              )}
              <input ref={fileRef} type="file" accept=".xlsx,.xls,.csv" onChange={handleImport} className="hidden" />
              <button onClick={()=>fileRef.current?.click()} disabled={importing} className="btn btn-ghost btn-sm">
          ...
          <div className="text-[10.5px] text-navy-400">
            Menampilkan <strong className="text-navy-800">{filtered.length}</strong> dari {employees.length} karyawan
          </div>
          <div className="card overflow-x-auto">
            <table className="tbl" style={{ minWidth: 800 }}>
              <thead>
                <tr>
                  <th>Karyawan</th><th>Posisi</th><th>Divisi</th><th>Entitas</th>
                  <th>Kontrak</th><th>Status</th><th>Join date</th><th>Masa kerja</th><th>End date</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(e => (
                  <tr key={e.id}>
                    <td>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="font-bold text-[12px]">{e.full_name}</div>
                          <div className="text-[10px] text-navy-400">{e.employee_id}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="text-[11px] text-navy-500">{fmtDate(e.join_date)}</td>
                    <td className="text-[11px]">{calcYoS(e.join_date)}</td>
                    <td className={cn('text-[11px]', e.end_date && new Date(e.end_date) < new Date(Date.now() + 30*86400000) ? 'text-red-600 font-bold' : 'text-navy-500')}>
                      {fmtDate(e.end_date) || '–'}
                    </td>
                  </tr>
                ))}
      ...
      <div className="grid grid-cols-2 gap-3">
        <InsightCard title="Creative division terbesar (15 orang)" text="15 dari 41 karyawan aktif ada di Creative — 37% dari total headcount. Perlu monitoring beban kerja dan retention strategy khusus." />
