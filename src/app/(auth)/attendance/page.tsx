...
export default async function AttendancePage() {
  ...
  const { data: leave } = await db.from('attendance_leave')
  ...
  const byType = approved.reduce((acc: any, l) => {
    acc[l.leave_type] = (acc[l.leave_type] || 0) + l.total_days; return acc
  }, {})

  const typeData = Object.entries(byType).sort((a: any, b: any) => b[1] - a[1])
  const maxType = typeData[0]?.[1] as number || 1

  const typeColors: Record<string, string> = {
    Tahunan: 'bg-teal-500', Sakit: 'bg-blue-500', Penting: 'bg-amber-500',
    Melahirkan: 'bg-purple-500', 'Cuti Bersama': 'bg-navy-500', Unpaid: 'bg-gray-400'
  }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="card">
            ...
            <div className="card-body space-y-2.5">
              {typeData.map(([type, days]) => (
                <InlineBar key={type} label={type as string} value={`${days} hari`}
                  pct={Math.round((days as number / maxType) * 100)}
                  color={typeColors[type as string] ?? 'bg-navy-500'} />
              ))}
            </div>
          </div>

          <div className="card">
            <div className="card-head"><span className="card-title">Absenteeism by division (estimasi)</span></div>
            ...
            <tbody>
              {(leave ?? []).map(l => (
                <tr key={l.id}>
                  ...
                  <td className="text-[11px] text-navy-500">{l.employee?.division}</td>
                  <td><Badge variant={l.leave_type === 'Sakit' ? 'blue' : l.leave_type === 'Tahunan' ? 'teal' : 'amber'}>{l.leave_type}</Badge></td>
                  <td className="text-[11px] text-navy-500">{fmtDate(l.start_date)}</td>
                  <td className="text-[11px] text-navy-500">{fmtDate(l.end_date)}</td>
                  <td className="font-bold text-[12px]">{l.total_days} hari</td>
        ...
        <InsightCard title="Social Media absensi tertinggi (8.2%) — berkorelasi dengan engagement rendah" text="Pola ketidakhadiran tertinggi di Social Media bersamaan dengan engagement 2.9/5 (terendah) dan turnover 14.3%. Ketiga indikator saling berkaitan — perlu program wellbeing dan review beban kerja." />
      </div>
    </div>
  )
}
