...
const TEAMS = [
  { div: 'Creative',     score: 3.8, members: 15, top: 3, pip: 1 },
  { div: 'Finance',      score: 4.3, members: 4,  top: 2, pip: 0 },
  { div: 'Marketing',    score: 3.6, members: 10, top: 2, pip: 1 },
  { div: 'Human Capital',score: 4.0, members: 3,  top: 1, pip: 0 },
  { div: 'Operations',   score: 3.3, members: 4,  top: 0, pip: 0 },
  { div: 'Social Media', score: 3.0, members: 7,  top: 0, pip: 2 },
  { div: 'IT & Systems', score: 3.9, members: 2,  top: 1, pip: 0 },
]
...
export default function PerformanceClient({ pip: initPip, employees, tna }: { pip: any[]; employees: any[]; tna: any[] }) {
  ...
  const avgScore = tna&&tna.length>0?(tna.reduce((s,t)=>s+(t.score??0),0)/tna.length).toFixed(1):'–'
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        <div className="card p-4 flex items-center gap-3">
          <div className="w-11 h-11 rounded-full bg-teal-50 flex items-center justify-center text-teal-600 font-bold text-lg">{avgScore!=='–'?avgScore:'–'}</div>
          <div><div className="text-[11px] text-slate-400">Avg training score</div><div className="text-[13px] font-semibold text-slate-800">dari 100</div></div>
        </div>
        <div className="card p-4 flex items-center gap-3">
          <div className="w-11 h-11 rounded-full bg-amber-50 flex items-center justify-center text-amber-600 font-bold text-lg">{pip.filter(p=>p.status==='Active').length}</div>
          <div><div className="text-[11px] text-slate-400">PIP / SP aktif</div><div className="text-[13px] font-semibold text-slate-800">monitoring</div></div>
        </div>
        <div className="card p-4 flex items-center gap-3">
          <div className="w-11 h-11 rounded-full bg-teal-50 flex items-center justify-center text-teal-600 font-bold text-lg">{pip.filter(p=>p.status==='Completed').length}</div>
          <div><div className="text-[11px] text-slate-400">SP selesai</div><div className="text-[13px] font-semibold text-slate-800">YTD 2026</div></div>
        </div>
        <div className="card p-4 flex items-center gap-3">
          <div className="w-11 h-11 rounded-full bg-blue-50 flex items-center justify-center text-blue-600 font-bold text-lg">{tna&&tna.length>0?Math.max(...tna.map(t=>t.score??0)):0}</div>
          <div><div className="text-[11px] text-slate-400">Score tertinggi</div><div className="text-[13px] font-semibold text-slate-800">dari 100</div></div>
        </div>
      </div>

      {/* Team cards — klik untuk masuk detail */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-[14px] font-semibold text-slate-800">Performance per Tim</h2>
          <span className="text-[11.5px] text-slate-400">Klik tim untuk lihat detail anggota, CSAT & KPI</span>
        </div>
        <div className="grid grid-cols-4 gap-3">
          {TEAMS.map(t => (
            <button key={t.div}
              onClick={() => router.push(`/performance/team/${encodeURIComponent(t.div)}`)}
            ...
            >
              ...
              <div className={`text-2xl font-bold mb-1 ${t.score>=4?'text-teal-600':t.score>=3.5?'text-amber-500':'text-red-500'}`}>
                {t.score}<span className="text-[13px] text-slate-300 font-medium">/5</span>
              </div>
              <div className="w-full bg-slate-100 rounded-full h-1.5 mb-3">
                <div className={`h-1.5 rounded-full ${t.score>=4?'bg-teal-500':t.score>=3.5?'bg-amber-400':'bg-red-400'}`}
      ...
      {/* Avg score bar chart */}
      <div className="card">
        <div className="card-head"><span className="card-title">Avg performance score by division</span></div>
        <div className="card-body space-y-2">
          {TEAMS.sort((a,b)=>b.score-a.score).map(t=>(
            <InlineBar key={t.div} label={t.div} value={`${t.score}/5`}
              pct={Math.round((t.score/5)*100)}
          ...
          <tbody>
            {pip.map(p=>(
              <tr key={p.id}>
                ...
                <td>
                  <button onClick={()=>toggleStatus(p)} className={`badge cursor-pointer ${p.status==='Active'?'badge-amber':'badge-teal'}`}>
                    {p.status}
                  </button>
                </td>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="form-label">Tipe</label>
                <select value={form.type} onChange={e=>fv('type',e.target.value)} className="form-input">
                  <option>SP1</option><option>SP2</option><option>SP3</option><option>PIP</option>
                </select>
              </div>
              <div><label className="form-label">Status</label>
                <select value={form.status} onChange={e=>fv('status',e.target.value)} className="form-input">
                  <option>Active</option><option>Completed</option><option>Extended</option><option>Terminated</option>
                </select>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="form-label">Rencana perbaikan</label><textarea value={form.improvement_plan} onChange={e=>fv('improvement_plan',e.target.value)} className="form-input h-16 resize-none" placeholder="Langkah-langkah perbaikan..."/></div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <InsightCard title="Finance: top performer (4.3/5)" text="Finance konsisten tertinggi. Extract best practice dan replikasikan ke divisi lain." color="bg-teal-700" titleColor="text-teal-100"/>
        <InsightCard title="Social Media: performa terendah (3.0/5)" text="Triple concern: performa rendah + absensi tinggi + turnover tinggi. Intervensi menyeluruh diperlukan." color="bg-red-900/70" titleColor="text-red-200"/>
      </div>
    </div>
  )
}
