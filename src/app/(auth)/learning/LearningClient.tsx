...
interface Props { tna: any[]; employees: any[] }

export default function LearningClient({ tna: init, employees }: Props) {
  const [tna, setTna] = useState(init)
  const [showModal, setShowModal] = useState(false)
  const [saving, setSaving] = useState(false)
  const [form, setForm] = useState({
    employee_id: '', training_name: '', training_category: 'Hard Skill',
    training_method: 'Online', quarter: 'Q2', year: 2026, target_date: '', status: 'Planned',
  })
  ...
  const totalHours = done.reduce((s, t) => s + (t.duration_hours ?? 8), 0)

  // Progress per employee
  const empProgress = employees.map(e => {
    const empTNA = tna.filter(t => t.employee_id === e.id)
    const empDone = empTNA.filter(t => t.status === 'Done').length
    const pct = empTNA.length > 0 ? Math.round(empDone / empTNA.length * 100) : 0
    const hasOverdue = empTNA.some(t => t.status === 'Overdue')
    return { ...e, total: empTNA.length, done: empDone, pct, hasOverdue }
  ...
  const statusColors: Record<string, string> = { Done: 'teal', 'In Progress': 'blue', Planned: 'gray', Overdue: 'red', Cancelled: 'gray' }
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-4 gap-3">
        ...
        <KPICard label="Total training hrs" value={`${totalHours} hr`} change="YTD 2026" changeType="flat" accent="bg-purple-400" />
      </div>
          ...
          <div className="card-body space-y-3">
            {empProgress.map(e => (
              <div key={e.id}>
                ...
                <div className="text-[9.5px] text-navy-400 mt-1">{e.done}/{e.total} training{e.hasOverdue ? ' · ⚠ overdue' : ''}</div>
              </div>
            ))}
            {empProgress.length === 0 && <div className="text-center text-navy-400 text-[11px] py-4">Belum ada data TNA</div>}
          </div>
        </div>
        ...
        <div className="col-span-2 space-y-3">
          <div className="flex justify-end">
            <button onClick={() => setShowModal(true)} className="btn btn-teal btn-sm">
              <Plus size={12} /> Input TNA
            </button>
          </div>
          <div className="card overflow-x-auto">
            <table className="tbl" style={{minWidth: 600}}>
              <thead><tr><th>Karyawan</th><th>Training</th><th>Kategori</th><th>Metode</th><th>Target</th><th>Status</th><th>Skor</th><th className="text-center">Aksi</th></tr></thead>
              <tbody>
                {tna.map(t => (
                  <tr key={t.id}>
                    <td>
                      <div className="font-bold text-[12px]">{t.employee?.full_name?.split(' ').slice(0,2).join(' ')}</div>
                      <div className="text-[10px] text-navy-400">{t.employee?.division}</div>
                    </td>
                    <td className="font-medium text-[11.5px]">{t.training_name}</td>
                    <td><Badge variant="gray">{t.training_category}</Badge></td>
                    <td className="text-[11px] text-navy-500">{t.training_method}</td>
                    <td className="text-[11px] text-navy-500">{fmtDate(t.target_date)}</td>
                    <td>
                      <select
                        value={t.status}
                        onChange={e => updateStatus(t.id, e.target.value)}
                        className={cn('badge border cursor-pointer outline-none bg-transparent text-[9.5px] font-bold appearance-none pr-1', `badge-${statusColors[t.status] ?? 'gray'}`)}
                      >
                        {['Planned','In Progress','Done','Overdue','Cancelled'].map(s => <option key={s}>{s}</option>)}
                      </select>
                    </td>
                    <td className={cn('font-bold text-[12px]', t.score ? 'text-teal-600' : 'text-navy-400')}>{t.score ? `${t.score}/100` : '–'}</td>
                    <td className="text-center"><button onClick={() => deleteTNA(t.id)} className="text-red-400 hover:text-red-600 text-[11px] font-bold">Hapus</button></td>
                  </tr>
                ))}
        ...
        <Modal title="Input TNA / Training" onClose={() => setShowModal(false)}>
          <div className="space-y-3">
            ...
            <div><label className="form-label">Nama training *</label>
              <input value={form.training_name} onChange={e=>setForm(p=>({...p,training_name:e.target.value}))} className="form-input" placeholder="e.g. Leadership Basics" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="form-label">Target tanggal</label>
                <input type="date" value={form.target_date} onChange={e=>setForm(p=>({...p,target_date:e.target.value}))} className="form-input" />
              </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <InsightCard title="Completion rate 61% — di bawah target 80%" text="Perlu akselerasi eksekusi training terutama di Q2–Q3. Dewi Rahayu dan Anisa Fitriani perlu priority follow-up karena sudah overdue." />
        <InsightCard title="Avg hours/employee baru 6.9hr — target 12hr/tahun" text="Setengah dari target annual training hours. Pertimbangkan format self-learning yang lebih fleksibel untuk meningkatkan jam training tanpa mengganggu produktivitas." color="bg-navy-600" titleColor="text-teal-200" />
      </div>
    </div>
  )
}
