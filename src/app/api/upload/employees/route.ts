...
export async function POST(req: NextRequest) {
  ...
  const mapped = rows.map((row: any) => ({
    ...
    employment_type: String(row['Tipe'] || row['employment_type'] || 'PKWTT').trim(),
    work_location:   row['Lokasi'] || row['work_location'] || 'Jakarta',
    gender:          row['Gender'] || row['gender'] || null,
    birth_date:      row['Tgl Lahir'] || row['birth_date'] || null,
    join_date:       row['Join Date'] || row['join_date'] || null,
    end_date:        row['End Date'] || row['end_date'] || null,
    status:          'active',
  ...
  const { data, error } = await db.from('employees').upsert(mapped, { onConflict: 'employee_id' }).select()
